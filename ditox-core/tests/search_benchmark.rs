...
fn benchmark_fts_queries() {
    ...
    let start_insert = Instant::now();
    {
        ...
        for i in 0..100000 {
            ...
            tx.execute(
                ...
                rusqlite::params![
                    e.id, "text", e.content, e.hash, e.byte_size as i64, 
                    e.created_at.to_rfc3339(), e.last_used.to_rfc3339(), 
                    e.favorite as i32, e.notes, e.collection_id
                ],
            ).unwrap();
        }
        tx.commit().unwrap();
    }
    println!("Insert time (5000 entries + FTS triggers): {:?}", start_insert.elapsed());
    ...
    println!("Search 'Entry' (matches ~all): {:?} (found {})", start.elapsed(), results.len());

    // Case B: Random prefix
    let start = Instant::now();
    let results = db.search_entries("Abc", 50).unwrap();
    println!("Search 'Abc' (random prefix): {:?} (found {})", start.elapsed(), results.len());
    
    // Case C: Specific ID-like string
    let start = Instant::now();
    let results = db.search_entries("Entry 4999", 50).unwrap();
    println!("Search 'Entry 4999' (specific): {:?} (found {})", start.elapsed(), results.len());
    
    // Case D: Filtered Search (Favorites)
    let start = Instant::now();
    let results = db.search_entries_filtered("Entry", 50, "favorite", None).unwrap();
    println!("Search Filtered 'Entry' + Favorite: {:?} (found {})", start.elapsed(), results.len());

    // Case E: Search in Notes
    let start = Instant::now();
    let results = db.search_entries("Special note", 50).unwrap();
    println!("Search Notes 'Special note': {:?} (found {})", start.elapsed(), results.len());
