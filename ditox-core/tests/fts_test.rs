...
fn test_fts_search() {
    ...
    // Insert test data
    db.insert(&create_entry("id-1", "rust programming language"))
        .unwrap();
    db.insert(&create_entry("id-2", "python programming"))
        .unwrap();
    ...
    db.insert(&create_entry("id-path", "C:\\dev\\personal\\ditox"))
        .unwrap();
