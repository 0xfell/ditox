...
fn favorites_and_prune() {
    ...
    bin().arg("--db").arg(&db).args(["prune","--max-items","0","--keep-favorites"]).assert().success();
    let after2 = bin().arg("--db").arg(&db).args(["list","--json"]).assert().success().get_output().stdout.clone();
    let after2_v: serde_json::Value = serde_json::from_slice(&after2).unwrap();
    assert_eq!(after2_v.as_array().unwrap().len(), 1);
    assert_eq!(after2_v.as_array().unwrap()[0].get("id").unwrap().as_str().unwrap(), first_id);
}

#[test]
#[cfg(target_os = "linux")]
fn clipboard_image_copy_guarded() {
    if std::env::var("DITOX_E2E_CLIPBOARD").ok().as_deref() != Some("1") { return; }
    ...
    // get id
    let out = String::from_utf8(bin().arg("--db").arg(&db).args(["list","--images"]).assert().success().get_output().stdout.clone()).unwrap();
    let id = out.split_whitespace().next().unwrap().to_string();
    // copy and read back via arboard
    bin().arg("--db").arg(&db).args(["copy"]).arg(&id).assert().success();
