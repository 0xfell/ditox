...
fn linux_roundtrip() {
    ...
    // wl-copy/paste
    let has_wl_copy = Command::new("wl-copy")
        .arg("--version")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    let has_wl_paste = Command::new("wl-paste")
        .arg("--version")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
    ...
    println!(
        "xclip: {} | xsel: {}",
        if has_xclip { "present" } else { "missing" },
        if has_xsel { "present" } else { "missing" }
    );
}
...
fn macos_roundtrip() {
    ...
    if !ok {
        println!("hint: try closing clipboard managers that may lock NSPasteboard.");
    }
}
...
fn windows_roundtrip() {
    ...
    if !ok {
        println!("hint: PowerShell Get-Clipboard must be available; try running as your desktop user session.");
    }
}
